...
import { isCelo, isFantom } from "../constants/tokens";
...
const QUOTE_GAS_OVERRIDES: { [chainId: number]: number } = {
  ...
  [SupportedChainId.FANTOM_TESTNET]: 50_000_000,
};
...
export function useClientSideV3Trade<TTradeType extends TradeType>(
  ...
  const useQuoterV2 = useMemo(
    () => Boolean(chainId && isFantom(chainId)),
    [chainId]
  );
  console.log("usequoterV2-->", useQuoterV2);
